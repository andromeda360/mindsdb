...
from mindsdb.integrations.handlers.google_analytics_handler.google_analytics_run_report_handler import GoogleAnalyticsHandler
...
class ReportTable(APITable):
    ...
    def __init__ (self, name: str, handler, **kwargs):
        ...
        self.handler : GoogleAnalyticsHandler = handler
        #self.handler: GoogleAnalyticsHandler = kwargs.get("handler")
    ...
    def _run_report_request(self, dimensions, metrics, date_range, filter_expression, property_id):
        ...
        # print("Connection handler: ", handler)
        # client = self.handler.create_connection()
        # print("Client: ", client)
    ...
    def validate_query_for_aggregations(self, query: ast.Select):
        ...
        query_string = str(query).upper()

        # use regex for more advanced cases
        if 'SUM(' in query_string:
            print("Error reached, query_string value?:", query_string)
            
            raise ValueError("Aggregation functions are not allowed.")
    ...
    def select(self, query: ast.Select) -> pd.DataFrame:
        ...
        # print(f"GROUP BY field dimensions: {grouped_fields}")
